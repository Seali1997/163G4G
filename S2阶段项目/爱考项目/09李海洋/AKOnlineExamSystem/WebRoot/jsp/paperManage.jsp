...
function showSelectCrePaper(){
	var width = (screen.width - 600) / 2;
	var height = (screen.height - 700) / 2;
	var dialog = window
			.open("jsp/createChoosePaper.jsp",
					"window",
	...
	<div>
		<fieldset id="marginTop">
			<table width="100%" align="center" id="marginTop" border="1"
				...
				<s:iterator value="pageBean.Items" var="paperone" status="status">
					<tr>
						<td>${status.count }</td>
						<td>${paperone.ptype }</td>
						<td>[${paperone.course.major }&nbsp;${paperone.course.stage }]<br>${paperone.course.csName }</td>
						<td>${paperone.pname }</td>
						<!-- TODO 显示所有班级 -->
						<td>
							<s:iterator value="#paperone.paperClasses" var="paperClass">
						...
						<!-- TODO 状态颜色 -->
						<td>
							<s:if test="#paperone.pstate == '未开考'">
								<font color="red">${paperone.pstate }</font>
							</s:if>
							<s:elseif test="#paperone.pstate == '考试中'">
								<font color="green">${paperone.pstate }</font>
							</s:elseif>
							<s:else>
								<font color="gray">${paperone.pstate }</font>
							</s:else>
						</td>
						<!-- TODO 根据状态显示不同的操作-->
						<td>
							<s:if test="#paperone.pstate == '未开考'">
								...
								<a href="javascript:showStartExam(${paperone.pid },${paperone.ptime })">开始考试</a>
							</s:if> 
							<s:elseif test="#paperone.pstate == '考试中'">
								<a href="paper/paper_endExam?paper.pid=${paperone.pid }">结束考试</a>
							</s:elseif> 
							<s:else>
								<a href="javascript:alert('同成绩管理！')">查看成绩</a>
							</s:else>
							<a href="javascript:alert('同成绩管理！')">查看试卷</a></td>
					</tr>
				</s:iterator>
			</table>
			...
			<div align="right" id="marginTop">
				...
				<a
					href="paper/paper_getPaperList?pageIndex=1">首页</a>&nbsp;
				<a
					href="paper/paper_getPaperList?pageIndex=${pageBean.preIndex }">上一页</a>&nbsp;
				<a
					href="paper/paper_getPaperList?pageIndex=${pageBean.nextIndex }">下一页</a>&nbsp;
				<a
					href="paper/paper_getPaperList?pageIndex=${pageBean.totalPage }">末页</a>
				<!-- <a
